...
function submitItem($idx, $name, $price, $details, $platform, $serverType, $buyLink) {
    $aes = new AES256();
    ...
    $q = $db->prepare("INSERT INTO `products` (`ownerId`, `name`, `price`, `details`, `platform`, `serverType`, `buyLink`) VALUES (?, ?, ?, ?, ?, ?, ?)");
    ...
    $q->bind_param('isissis', $idx, $name, $price, $details, $platform, $serverType, $buyLink);
    ...
    if ($q->affected_rows == 1) {
        ...
        header("Location: ../create-item/?err=" . urlencode($aes->encrypt("Item created successfully.", "secretkey")));
        exit(); // Exit script after redirection
    }

    // Redirecting with error message if insertion failed
    header("Location: ../create-item/?err=" . urlencode($aes->encrypt("An unknown error occurred.", "secretkey")));
    exit(); // Exit script after redirection
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $serverType = strip_tags($_POST['serverType'] ?? '');
	$buyLink = strip_tags($_POST['buyLink'] ?? '');

    // Checking if all required fields are not empty
    if (!empty($name) && !empty($price) && !empty($details) && !empty($platform) && !empty($serverType) && !empty($buyLink)) {
        // Calling function to submit item details and exiting script after redirection
        submitItem($idx, $name, $price, $details, $platform, $serverType, $buyLink);
    } else {
